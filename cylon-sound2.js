...
window.BEEP_VOLUME = (typeof window.BEEP_VOLUME !== 'undefined') ? window.BEEP_VOLUME : 1;
...
window.beep = function(frequency = 800, duration = 200, volume) {
  try {
    ...
    var vol = (typeof volume === 'number') ? volume : (window.BEEP_VOLUME || 1);
...
window.volume = function(level) {
  ...
  if (arguments.length === 0) { return (typeof window.BEEP_VOLUME === 'number') ? window.BEEP_VOLUME : 1; }
  var prev = (typeof window.BEEP_VOLUME === 'number') ? window.BEEP_VOLUME : 1;
  var v = Number(level);
  if (isNaN(v)) v = 0;
...
// ──── Multi-Track & Chord Support - now supports [CEG]4 syntax for chords ────────────────
...
window.noteFrequencies = noteFrequencies;
window.playNote = playNote;
window.currentTune = currentTune;
window.playTune = playTune;
window.parseMusicString = parseMusicString;
window.playNextNote = playNextNote;
window.stopTune = stopTune;
window.loopTune = loopTune;
window.loopNotes = loopNotes;
window.playMelody = playMelody;
window.playNotes = playNotes;
window.parseGWBasicString = parseGWBasicString;
